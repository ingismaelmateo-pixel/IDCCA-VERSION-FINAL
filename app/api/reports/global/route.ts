...
import { eq, and, gte, lte, count, sum, sql, desc } from "drizzle-orm"; // 👈 Agregado 'desc'
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 6. Nuevos Miembros y Visitantes
    ...
    // ============================================================
    // 🔥 CORRECCIÓN 1: Obtener el desglose de Ingresos (Pie Chart)
    // ============================================================
    ...
    // ============================================================
    // 🔥 CORRECCIÓN 2: Obtener el Top 5 Miembros (Requiere 'desc')
    // ============================================================
    const topMembers = await db
      ...
      .orderBy(desc(sum(financialTransactions.amount))) // 👈 Usando 'desc' correctamente
      .limit(5);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        // 👈 Agregamos el objeto completo para el gráfico
        incomeDistribution: {
          tithe: parseFloat(titheResult[0]?.total || "0"),
          offering: parseFloat(offeringResult[0]?.total || "0"),
          donation: parseFloat(donationResult[0]?.total || "0"),
        },
        topMembers,
      }
    });
